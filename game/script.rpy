...
default current_crimping_tipe = "T568B"
...
# (Definisi video intro lama dihapus)
        ...
        frame:
            ...
            vbox:
                ...
                vbox:
                    spacing 5
                    hbox:
                        text "NET-PRO // HARDWARE SIMULATOR v1.0" size 12 color "#00FF00" xalign 0.0
                        text "SYS: ONLINE" size 12 color "#00FF00" xalign 1.0
                    text "==========================================================================" size 14 color "#00FF00"
                text ">> MODULE: RJ-45 CABLE CRIMPING" size 24 color "#00FFFF" bold True
                text ">> PROTOCOL: [current_crimping_tipe] STANDARD" size 14 color "#00FFFF"

                ## Referensi standar T568B ala manual sheet
                frame:
                    background Solid("#1E293B")
                    padding (15, 10)
                    vbox:
                        spacing 5
                        text "DATABASE PANDUAN WARNA [[[current_crimping_tipe]]]:" size 12 color "#FACC15" bold True
                        hbox:
                            spacing 5
                            for i, nama in enumerate(get_hint_colors(current_crimping_tipe)):
                                frame:
                                    background Solid(kabel_warna_bg(get_hint_ids(current_crimping_tipe)[i]))
                                    padding (4, 4)
                                    xsize 92
                                    text nama size 12 color "#FFFFFF" bold True xalign 0.5

                ## Grid slot (Kabel yang disusun)
                vbox:
                    spacing 5
                    text "[[ CONNECTOR PIN ASSIGNMENT ]" size 14 color "#94A3B8" bold True
                    ...
                    frame:
                        ...
                        hbox:
                            ...
                            for i in range(8):
                                vbox:
                                    spacing 5
                                    # Pin Kuningan (Konektor)
                                    frame:
                                        xsize 80
                                        ysize 15
                                        background Solid("#FBBF24")
                                    # Kabel
                                    frame:
                                        xsize 80
                                        ysize 100
                                        background Solid(slot_color(crimping_slot[i]))
                                        text (slot_label(crimping_slot[i]) if crimping_slot[i] != 0 else "NO\nLINK") size 12 color "#FFFFFF" bold True xalign 0.5 yalign 0.5 text_align 0.5
                                    text f"PIN {i+1}" size 12 color "#4ADE80" bold True xalign 0.5

                ## Panel pilih kabel
                vbox:
                    spacing 5
                    text "[[ SELECT WIRE CORE ]" size 14 color "#94A3B8"
                            ...
                            hbox:
                                ...
                                for i in range(row_start, row_start + 4):
                                    button:
                                        xsize 140
                                        ysize 40
                                        background Solid(kabel_warna_bg(i+1))
                                        action Function(add_kabel_to_next_slot, i+1)
                                        text slot_label(i+1).upper() size 14 color "#FFFFFF" bold True xalign 0.5 yalign 0.5

                ## Tombol aksi ala Control Panel
                null height 5
                hbox:
                    spacing 15
                    xalign 0.5
                    button:
                        action SetVariable("crimping_slot", [0,0,0,0,0,0,0,0])
                        background Solid("#991B1B") 
                        xsize 150
                        ysize 40
                        text "[[ SYS RESET ]" color "#FFFFFF" size 14 bold True xalign 0.5 yalign 0.5

                    button:
                        action Function(hapus_slot_terakhir)
                        background Solid("#B45309") 
                        xsize 150
                        ysize 40
                        text "[[ REMOVE ]" color "#FFFFFF" size 14 bold True xalign 0.5 yalign 0.5

                    button:
                        action [Hide("crimping_game"), Return("check")]
                        background Solid("#0369A1") 
                        xsize 250
                        ysize 40
                        text "[[ EXECUTE TESTER ]" color "#FFFFFF" size 14 bold True xalign 0.5 yalign 0.5
...
screen crimping_result(benar):
    ...
    frame:
        ...
        frame:
            background Solid(("#064E3B" if benar else "#450A0A"))
            padding (30, 25)
            vbox:
                spacing 10
                text ">> TESTER LOGS:" size 14 color "#94A3B8"
                text "==================================================" size 14 color "#94A3B8"
                
                if benar:
                    text "[[ + ] PING LOOPBACK ... OK" size 14 color "#4ADE80"
                    text "[[ + ] CABLE PAIRING ... [current_crimping_tipe] MATCHED" size 14 color "#4ADE80"
                    text "[[ + ] LINK SPEED ...... 1000 Mbps" size 14 color "#4ADE80"
                    null height 5
                    text "✅ VALIDATION SUCCESS" size 30 color "#4ADE80" bold True xalign 0.5
                    text "Koneksi stabil. Kabel UTP siap untuk di-deploy!" size 14 color "#F8FAFC" xalign 0.5
                else:
                    text "[[ - ] PING LOOPBACK ... TIMEOUT" size 14 color "#F87171"
                    text "[[ - ] CABLE PAIRING ... MISMATCH DETECTED" size 14 color "#F87171"
                    text "[[ - ] LINK SPEED ...... 0 Mbps" size 14 color "#F87171"
                    null height 5
                    text "❌ VALIDATION FAILED" size 30 color "#F87171" bold True xalign 0.5
                    text "Urutan pin salah. Cek standar [current_crimping_tipe]!" size 14 color "#F8FAFC" xalign 0.5
                ...
                button:
                    ...
                    background Solid(("#166534" if benar else "#991B1B"))
                    text "[[ ACKNOWLEDGE ]" color "#FFFFFF" size 14 bold True xalign 0.5 yalign 0.5
...
init python:
    ...
    _kabel_colors = {
        0: "#111827",  # Slot kosong
        1: "#E65100",  # Orange Putih
        2: "#BF360C",  # Orange
        3: "#2E7D32",  # Hijau Putih
        4: "#0D47A1",  # Biru
        5: "#1976D2",  # Biru Putih
        6: "#1B5E20",  # Hijau
        7: "#5D4037",  # Coklat Putih
        8: "#3E2723",  # Coklat
    }

    def slot_label(val):
        return _kabel_names.get(val, "NONE")

    def slot_color(val):
        return _kabel_colors.get(val, "#111827")

    def kabel_warna_bg(idx):
        return _kabel_colors.get(idx, "#111827")
    ...
    def check_crimping():
        if current_crimping_tipe == "T568A":
            correct = [3, 6, 1, 4, 5, 2, 7, 8]
        else:
            correct = [1, 2, 3, 4, 5, 6, 7, 8]
        return list(crimping_slot) == correct

    def get_hint_colors(tipe):
        if tipe == "T568A":
            return ["1.P.Hju", "2.Hijau", "3.P.Org", "4.Biru", "5.P.Bru", "6.Org", "7.P.Ckl", "8.Coklat"]
        return ["1.P.Org", "2.Org", "3.P.Hju", "4.Biru", "5.P.Bru", "6.Hijau", "7.P.Ckl", "8.Coklat"]

    def get_hint_ids(tipe):
        if tipe == "T568A":
            return [3, 6, 1, 4, 5, 2, 7, 8]
...
label start:
    $ persistent.chapter_unlocked = max(persistent.chapter_unlocked or 1, 1)
    ...
    # (Video intro lama dilewati langsung ke narasi awal)
...
label quest2_t568a:
    ...
    narasi "═══ QUEST 2 (EXTRA): MINI-GAME CRIMPING KABEL T568A ═══"

    mentor "Bagus, kamu sudah menguasai T568B. Tapi sebagai teknisi jaringan, kamu juga harus tahu standar T568A."
    mentor "T568A biasanya digunakan pada infrastruktur tertentu, dan juga sebagai salah satu ujung kabel Crossover."
    mentor "Ini urutannya: Putih Hijau, Hijau, Putih Orange, Biru, Putih Biru, Orange, Putih Coklat, Coklat."

    show rafi bingung at left with dissolve
    rafi "Waduh, beda lagi? Yang ketukar apanya Pak?"
    mentor "Gampang! Pin 1 & 2 (Orange) ditukar dengan Pin 3 & 6 (Hijau). Sisanya sama persis."
    rafi "Ohh... Putih Hijau dan Hijau maju ke depan, Putih Orange dan Orange mundur ke posisi 3 dan 6."
    hide rafi with dissolve

    mentor "Tepat sekali. Sekarang [nama_mc], coba buat konfigurasi T568A."

label quest2_t568a_mulai:

    $ current_crimping_tipe = "T568A"
    $ tries_q2 += 1
    $ crimping_slot = [0,0,0,0,0,0,0,0]
    ...
    if check_crimping():
        jump quest2_t568a_benar
    else:
        jump quest2_t568a_salah

label quest2_t568a_benar:
    ...
    sistem "[[ CABLE TEST ] ✅ T568A — Semua 8 pin terhubung dengan benar!"
    sistem "[[ HASIL ] Benar — Putih Hijau, Hijau, Putih Orange, Biru, Putih Biru, Orange, Putih Coklat, Coklat ✓"

    mentor "Sempurna! Kamu sekarang bisa membuat kabel Straight-through (T568B-T568B) maupun Crossover (T568A-T568B)."
    
    jump quest3

label quest2_t568a_salah:

    scene bg lab with dissolve
    show screen hud_score
    show mentor tegas at right with dissolve

    sistem "[[ CABLE TEST ] ❌ GAGAL — Urutan pin tidak sesuai standar T568A"
    sistem "[[ HASIL ] Salah — Terdapat urutan kabel yang tidak tepat"

    mentor "Masih salah. Ingat, tukar pasangan Orange dengan Hijau dari standar T568B."
    mc "Waduh, harus diulang..."

    if tries_q2 >= 5:
        mentor "Sudah dicoba [tries_q2] kali. Ayo fokus — urutannya: Putih Hijau, Hijau, Putih Orange, Biru, Putih Biru, Orange, Putih Coklat, Coklat."
    else:
        mentor "Coba lagi ya. Perhatikan urutannya dengan teliti."

    jump quest2_t568a_mulai
...
label chapter2:
    $ persistent.chapter_unlocked = max(persistent.chapter_unlocked or 1, 2)
...
label chapter3:
    $ persistent.chapter_unlocked = max(persistent.chapter_unlocked or 1, 3)
...
label chapter4:
    $ persistent.chapter_unlocked = max(persistent.chapter_unlocked or 1, 4)
...
label chapter5:
    $ persistent.chapter_unlocked = max(persistent.chapter_unlocked or 1, 5)
