...
screen navigation():
    ...
    vbox:
        ...
        if main_menu:
            ...
            textbutton _("Play New Game") action Start()
...
screen main_menu():
    ...
    add Transform(gui.main_menu_background, xsize=1920, ysize=1080)

    ## MENU KUSTOM SESUAI REFERENSI GUI (Posisi di Kanan Samping Logo)
    vbox:
        xalign 0.72   # Geser ke kanan di samping logo NetPro
        yalign 0.50   # Sejajarkan di tengah vertikal di samping logo
        spacing 8     # Jarak antar tombol saling berdekatan dan padat

        # Tombol-tombol menu menggunakan gambar (imagebutton)
        imagebutton idle "gui/Mulai.png" at main_menu_btn_zoom action Start() # Memulai game baru
        imagebutton idle "gui/Lanjutkan.png" at main_menu_btn_zoom action ShowMenu("load") # Melanjutkan game (Load)
        imagebutton idle "gui/PilihChapter.png" at main_menu_btn_zoom action ShowMenu("chapter_select") # Memilih chapter game
    ...
    # Judul bawaan Ren'Py dimatikan sepenuhnya agar tidak mengganggu logo di Wallpaper
    if False:
        vbox:
...
default persistent.chapter_unlocked = 1

screen chapter_select():
    tag menu
    
    # Menambahkan latar belakang menu utama (secara otomatis di-resize)
    add Transform(gui.main_menu_background, xsize=1920, ysize=1080)
    
    # Tambahkan overlay gelap transparan agar lebih estetik dan teks mudah dibaca
    add Solid("#051226ee")
    ...
    vbox:
        ...
        # Judul Menu
        text "PILIH CHAPTER" size 45 bold True color "#00E5FF" xalign 0.5 outlines [ (2, "#000000", 0, 0) ]
        
        # Penjelasan singkat
        text "Selesaikan chapter sebelumnya untuk membuka chapter baru." size 18 color "#a5b4fc" xalign 0.5 yoffset -5
        ...
        vbox:
            ...
            textbutton "Chapter 1: Dasar Jaringan & Crimping Kabel" action Start("start") xalign 0.5 style "chapter_button"
            
            # CHAPTER 2
            if (persistent.chapter_unlocked or 1) >= 2:
                textbutton "Chapter 2: VLAN & Segmentasi Jaringan" action Start("chapter2") xalign 0.5 style "chapter_button"
            else:
                textbutton "Chapter 2: [[TERKUNCI]]" action None xalign 0.5 style "chapter_button_locked"
                
            # CHAPTER 3
            if (persistent.chapter_unlocked or 1) >= 3:
                textbutton "Chapter 3: WAN, NAT, & Security Firewall" action Start("chapter3") xalign 0.5 style "chapter_button"
            else:
                textbutton "Chapter 3: [[TERKUNCI]]" action None xalign 0.5 style "chapter_button_locked"
                
            # CHAPTER 4
            if (persistent.chapter_unlocked or 1) >= 4:
                textbutton "Chapter 4: Server Administrator & VoIP" action Start("chapter4") xalign 0.5 style "chapter_button"
            else:
                textbutton "Chapter 4: [[TERKUNCI]]" action None xalign 0.5 style "chapter_button_locked"
                
            # CHAPTER 5
            if (persistent.chapter_unlocked or 1) >= 5:
                textbutton "Chapter 5: Advanced Management & Disaster Recovery" action Start("chapter5") xalign 0.5 style "chapter_button"
            else:
                textbutton "Chapter 5: [[TERKUNCI]]" action None xalign 0.5 style "chapter_button_locked"
                
        # Tombol Kembali ke Menu Utama
        textbutton "Kembali ke Menu Utama" action ShowMenu("main_menu") xalign 0.5 yoffset 30 style "chapter_back_button"

# Gaya khusus untuk tombol chapter agar terlihat estetik dan senada dengan logo biru
style chapter_button is button:
    background Frame("#1e293b", 6, 6)
    hover_background Frame("#3b82f6", 6, 6)
    padding (35, 12)
    xminimum 650
    xalign 0.5
...
style chapter_button_locked is button:
    background Frame("#0f172a", 6, 6)
    padding (35, 12)
    xminimum 650
    xalign 0.5

style chapter_button_locked_text is button_text:
    color "#64748B"
    size 22
    bold True

style chapter_back_button is button:
    background Frame("#334155", 6, 6)
    hover_background Frame("#ef4444", 6, 6)
    padding (20, 8)
    xminimum 250
    xalign 0.5

style chapter_back_button_text is button_text:
    color "#CBD5E1"
    hover_color "#ffffff"
    size 18
    bold True

style main_menu_chapter_btn is button:
    background Frame("#1e293b", 6, 6)
    hover_background Frame("#3b82f6", 6, 6)
    padding (15, 8)
    xminimum 280
...
style main_menu_chapter_btn_text is button_text:
    color "#E2E8F0"
    hover_color "#ffffff"
    size 20
    bold True
...
screen game_menu(title, scroll=None, yinitial=0.0, spacing=0):
    ...
    if main_menu:
        ...
        add Transform(gui.game_menu_background, xsize=1920, ysize=1080)
...
screen about():
    ...
    use game_menu(_("Tentang"), scroll="viewport"):
        ...
        vbox:
            ...
            text _("NetPro: Magang Jaringan adalah game edukasi Visual Novel yang dibuat untuk membantu pembelajaran Teknik Komputer dan Jaringan (TKJ) melalui pengalaman interaktif berbasis cerita.")

            text _("Pemain akan merasakan simulasi dunia kerja sebagai teknisi jaringan, mulai dari dasar seperti instalasi kabel hingga menangani permasalahan jaringan dan keamanan.\n")

            label _("Dikembangkan oleh:")
            text _("Eko Muhammad Rizki\n")

            label _("Game ini dibuat sebagai media pembelajaran untuk:")
            text _("• Mempermudah pemahaman konsep jaringan komputer")
            text _("• Menggabungkan teori dengan praktik simulasi")
            text _("• Memberikan gambaran dunia kerja IT secara nyata\n")

            text _("Dikembangkan menggunakan Ren'Py Engine\n")

            text _("Terima kasih telah memainkan game ini.")
